...
```text
GitHub Release published (tag vX.Y.Z, e.g. v0.4.0)
        ↓
...
Before creating and publishing any release tag (`vX.Y.Z`, e.g. `v0.4.0`):

- [ ] `main` branch CI is completely green (`.github/workflows/ci.yml`).
- [ ] Release preparation PR (e.g. `release/v0.4.0`) is reviewed and merged to `main`.
- [ ] Post-merge CI on `main` for the release preparation merge commit is verified green.
- [ ] Package version is aligned across `pyproject.toml` and `src/fa_redact/__init__.py` (`0.4.0`).
- [ ] `CHANGELOG.md` has a finalized section `[0.4.0] - 2026-09-08` and a fresh `[Unreleased]` section.
...
- [ ] Git tag `v0.4.0` does NOT already exist locally or remotely (`git tag -l`, `git ls-remote --tags origin`).
- [ ] Target version `0.4.0` is NOT already published on PyPI.
...
3. Configure the release:
   ...
   - **Tag**: `v0.4.0` (must match exact lowercase `v<version>` syntax)
   - **Title**: `fa-redact v0.4.0`
   - **Description**: Copy release notes directly from `CHANGELOG.md` `[0.4.0]` section.
...
- [ ] Target release is live on PyPI at `https://pypi.org/project/fa-redact/0.4.0/`.
...
- [ ] Public API verification in clean environment:
  ...
  ```
- [ ] Update `PROJECT_STATUS.md` to reflect `v0.4.0` as the latest published release.
