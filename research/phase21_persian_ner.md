...
- **Persian Clinical & Health Text Resources:** Persian clinical/health text resources exist publicly (such as medical question-answering and healthcare consultation datasets), but Phase 21 did not identify a public, gold-standard Persian clinical `PERSON` de-identification benchmark suitable for this project. Public medical corpora focus on clinical terminology (diseases, symptoms, pharmacology) or patient Q&A rather than gold personal-name de-identification in clinical EHR or discharge summaries. This is a finding of the Phase 21 search, not a claim that no such resource can exist.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **1. Heuristic / Dictionary / Regex** | Static name lists + Honorific prefixes + Suffix rules | Standard Library (`re`, `set`) — **0 dependencies** | N/A — conceptual approach | 100% Offline | Poor (severe false positives on common words; misses out-of-vocabulary names) | Minimal CPU overhead | **REJECTED.** Unacceptable false positive / false negative rates. |
...
| **5. DistilBERT-fa NER** (`HooshvareLab/distilbert-fa-zwnj-base-ner`) | 6-layer Transformer (66M params, distilled) | `transformers`, `torch`, `tokenizers` | UNCLEAR / REQUIRES VERIFICATION (Base model is Apache-2.0, but fine-tuned NER checkpoint lacks explicit license declaration on its repository) | 100% Offline with local weights | **95.07% Overall F1**, **95.85% PER F1** on combined Persian NER dataset | Reduced parameter count relative to BERT-base; actual CPU latency and memory behavior require benchmarking | **CANDIDATE LIGHTWEIGHT.** Compact architecture with PER-specific reporting. |
| **6. ONNX Runtime Exported ParsBERT / DistilBERT** | Pre-quantized INT8 / FP32 ONNX graph + Fast Subword Tokenizer | `onnxruntime` + Python standard library | Apache-2.0 (Model permitting) / MIT (ONNX Runtime) | 100% Offline | Expected close to PyTorch baseline; requires empirical benchmarking | Reduced dependency footprint compared to full PyTorch; actual performance requires prototype benchmarking | **CANDIDATE ARCHITECTURE.** Requires prototype validation for Python matrix compatibility. |
...
| **WikiANN (fa)** | Dataset | UNKNOWN / REQUIRES VERIFICATION (Canonical `unimelb-nlp/wikiann` metadata declares unknown; script Apache-2.0 covers loader code only) | **YES** (Academic reference) | **UNCLEAR / REQUIRES VERIFICATION** (Technically downloadable; permitted use terms require verification) | **NO** (Unverified license / silver data) | **NO** (Cannot bundle or redistribute unverified corpus) |
...
| **DistilBERT-fa Weights** (`HooshvareLab`) | Model Weights | UNCLEAR / REQUIRES VERIFICATION (Fine-tuned NER checkpoint lacks explicit license tag; base model is Apache-2.0) | **YES** | **UNCLEAR / REQUIRES VERIFICATION** (Technically downloadable; permitted use/integration requires checkpoint license clarification) | **NO** (Large binary files do not belong in git) | **NO / REQUIRES VERIFICATION** (Unclear license prevents unverified redistribution/integration) |
...
- **The exact numerical magnitude of this domain shift is UNKNOWN** from current evaluation, as Phase 21 did not identify or execute a public, gold-standard Persian clinical PERSON de-identification benchmark suitable for this project. This is a finding of the Phase 21 search, not a claim that no such resource can exist.
